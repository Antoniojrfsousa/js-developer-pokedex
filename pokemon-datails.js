    ...
    .then((pokemonData) => {
        ...
        <div class="card ${pokemonData.type}">
            ...
            <div class="bottom">
              ...
              <button id="voltarIndex" onClick=home()>Back</button>
            </div>
        </div>
        `;
});
...
pokeApi.getPokemons().then((pokemonsList = []) => {
         elementList.innerHTML += pokemonsList.map(convertPokeApiDetailToPokemon).join("");
});
...
document.getElementById("voltario").addEventListener("click", function () {
    window.location.href = "index.html";
});